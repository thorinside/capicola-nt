...
For real-time safety, the audio callback never scans the catalogue, changes parameter definitions, or reads the SD card. It only reads and wraps the fixed memory buffer and runs Capicola. Removing the card after a successful load does not interrupt the loop. Inserting or remounting the card does not automatically recover a failed load from the audio callback; confirm `Sample` or change Folder to refresh the catalogue and load again.
